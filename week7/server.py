...
def request_vote_worker_thread(id_to_request):
    ...
    try:
        resp = stub.RequestVote(pb2.RequestVoteArgs(
            term=state['term'], 
            node_id=state['id'],
            last_index=logs[-1].index,
        ...
        try:
            if heartbeat_events[id_to_request].wait(timeout=0.5):
                ...
                resp = stub.AppendEntries(pb2.AppendEntriesArgs(
                    term=state['term'], 
                    node_id=state['id'],
                    prev_index=logs[-1].index,
                    prev_term=logs[-1].term,
                    entries=logs,
...
class Handler(pb2_grpc.RaftNodeServicer):
    def RequestVote(self, request, context):
        ...
        with state_lock:
            ...
            if state['lastLogIndex'] > request.last_index:
                return pb2.ResultWithTerm(**reply)
            if state['lastLogIndex'] == request.last_index and logs[state['lastLogIndex']-1].term != request.last_term:
                return pb2.ResultWithTerm(**reply)
    ...
    def AppendEntries(self, request, context):
        ...
        with state_lock:
            ...
            not_ok = True
            for log in logs:
                if log.index == request.prev_index:
                    not_ok = False
            if not_ok:
                return pb2.ResultWithTerm(**reply)
